...
final class StubSearchReadyViewModel: SearchReadyViewModelProtocol {
    // Exposed relays to control outputs in tests
    let popularPlaces$ = BehaviorRelay<[Place]>(value: [])
    let recentSearches$ = BehaviorRelay<[RecentSearchItem]>(value: [])
    let errorToFetchPlaces$ = BehaviorRelay<NetworkError?>(value: nil)
    let errorToFetchRecentSearches$ = BehaviorRelay<RecentSearchError?>(value: nil)
    let isFetching$ = BehaviorRelay<Bool>(value: false)

    // Input capture for assertions
    private let disposeBag = DisposeBag()
    var lastRecentTappedId: String?
    var lastPopularTappedId: String?
    var didTapMoreRecent = false
    var didTapMorePopular = false

    func transform(input: SearchReadyViewModel.Input) -> SearchReadyViewModel.Output {
        input.recentSearchCellTapped$
            .subscribe(onNext: { [weak self] id in self?.lastRecentTappedId = id })
            .disposed(by: disposeBag)

        input.popularPlaceCellTapped$
            .subscribe(onNext: { [weak self] id in self?.lastPopularTappedId = id })
            .disposed(by: disposeBag)

        input.moreRecentSearchesButtonTapped$
            .subscribe(onNext: { [weak self] in self?.didTapMoreRecent = true })
            .disposed(by: disposeBag)

        input.morePopularPlacesButtonTapped$
            .subscribe(onNext: { [weak self] in self?.didTapMorePopular = true })
            .disposed(by: disposeBag)

        // viewLoaded$ / refetchButtonTapped$ 는 테스트에서 직접 상태를 주입하므로 별도 처리 없음

        return .init(
            popularPlaces$: popularPlaces$.asObservable(),
            recentSearches$: recentSearches$.asObservable(),
            errorToFetchPlaces$: errorToFetchPlaces$.asObservable(),
            errorToFetchRecentSearches$: errorToFetchRecentSearches$.asObservable(),
...
final class SearchReadyViewControllerTests: XCTestCase {
    ...
    var mockVM: StubSearchReadyViewModel!

    override func setUp() {
        super.setUp()
        mockVM = StubSearchReadyViewModel()
